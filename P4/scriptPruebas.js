      ...
      if (document.querySelectorAll('.activeBox').length > 1) {
        ...
        document.querySelectorAll('.activeBox')[1].innerHTML){
          ...
          if (document.querySelectorAll('.matchItem').length == emojis.length){
            ...
            Swal.fire({
              title: 'SUCCESS!',
              text: `Your gaming time is: ${state.totalTime} Movements: ${state.totalTime}`,
              icon: 'success',
            });
            //alert(`You win in ${state.totalTime} seconds with ${state.totalTime} moves!`);
